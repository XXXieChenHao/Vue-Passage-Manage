...
- 传参过程
  - router.js 配置路由时 {path: '/ 地址 /: 参数名'}
  - 在指定页面接收参数 使用 `this.$route.params.参数名`

#### 问题 bug：

1、 修改文章错误，每次修改都会添加一篇新的文章

- 问题分析：
  - 新增文章不需要传递文章 id，修改文章则需要在请求地址后拼接文章 id
  - 新增文章使用 POST 请求，而修改文章需要使用 PUT 请求
...
- 子向父传参
  ...
  - 在父组件中为使用的子组件添加自定义事件，在子组件中使用 `$emit`调用该方法并且传参

  ```
  // --- 在父组件文件中
  <子组件标签 @自定义事件名称="自定义方法名"></子组件标签>
   methods:{
  	 自定义方法名(arg1,arg2){xxxx}
   }
  
  // --- 在子组件文件中
   this.$emit('与父组件相同的自定义事件名称',参数1,参数2...)
...
- 在 watch 中使用箭头函数，发现报错找不到 callback，监听器本身就是 Vue 对象调用，所以改为箭头函数后，this 为 Vue 对象所在执行环境，但 Vue 已在顶层，所以 undefined
...
```
<el-pagination 
:current-page.sync="选中页码" 
@current-change="页码选择函数" 
layout="prev, pager, next" 
:total="总页数"></el-pagination>
```
